	...
	async execute(interaction) {
      ...
      if (corp == "corp1") {
        if (!interaction.member.roles.cache.some(role => role.id === '1068550480642445422')) {
          Sublogs.send(`**법인에서개인이체 실패** <@${interaction.user.id}>님이 서대문구청 권한이 없어서 명령에 실패했습니다.`)
        ...
        if (!interaction.member.roles.cache.some(role => role.id === '1068550491723796480')) {
          Sublogs.send(`**법인에서개인이체 실패** <@${interaction.user.id}>님이 DB손해보험 권한이 없어서 명령에 실패했습니다.`)
          return interaction.reply({ ephemeral: true, embeds: [embed1]})}
      } else if (corp == "corp3") {
        if (!interaction.member.roles.cache.some(role => role.id === '1068550494278123630')) {
          Sublogs.send(`**법인에서개인이체 실패** <@${interaction.user.id}>님이 SBS 권한이 없어서 명령에 실패했습니다.`)
          return interaction.reply({ ephemeral: true, embeds: [embed1]})}
      } else if (corp == "corp4") {
        if (!interaction.member.roles.cache.some(role => role.id === '1068550495674847243')) {
          Sublogs.send(`**법인에서개인이체 실패** <@${interaction.user.id}>님이 서대문은행 권한이 없어서 명령에 실패했습니다.`)
          return interaction.reply({ ephemeral: true, embeds: [embed1]})}
      } else if (corp == "corp5") {
        if (!interaction.member.roles.cache.some(role => role.id === '1068550497017020507')) {
          Sublogs.send(`**법인에서개인이체 실패** <@${interaction.user.id}>님이 이세계아이돌카페 권한이 없어서 명령에 실패했습니다.`)
          return interaction.reply({ ephemeral: true, embeds: [embed1]})}
      } else if (corp == "corp6") {
        if (!interaction.member.roles.cache.some(role => role.id === '1068550817537347634')) {
          Sublogs.send(`**법인에서개인이체 실패** <@${interaction.user.id}>님이 카페쉬는곳 권한이 없어서 명령에 실패했습니다.`)
          return interaction.reply({ ephemeral: true, embeds: [embed1]})}
      } else if (corp == "corp7") {
        if (!interaction.member.roles.cache.some(role => role.id === '1068551495190065202')) {
          Sublogs.send(`**법인에서개인이체 실패** <@${interaction.user.id}>님이 서대문경찰서 권한이 없어서 명령에 실패했습니다.`)
          return interaction.reply({ ephemeral: true, embeds: [embed1]})}
      } else if (corp == "corp8") {
        if (!interaction.member.roles.cache.some(role => role.id === '1068551452382986381')) {
          Sublogs.send(`**법인에서개인이체 실패** <@${interaction.user.id}>님이 서대문소방서 권한이 없어서 명령에 실패했습니다.`)
          return interaction.reply({ ephemeral: true, embeds: [embed1]})}
      } else if (corp == "corp9") {
      ...
      //기타 조건 확인
      if (!findMe) {
        Sublogs.send(`**법인에서개인이체 실패** <@${interaction.user.id}>님이 보내는분 계좌가 없어서 실패했습니다.`)
        return interaction.reply({ ephemeral: true, embeds: [embed1]})}
      if (!findSend) {
        Sublogs.send(`**법인에서개인이체 실패** <@${interaction.user.id}>님이 받는분 계좌가 없어서 명령에 실패했습니다.`)
      ...
      if (betting <= 0) {
        Sublogs.send(`**법인에서개인이체 실패** <@${interaction.user.id}>님이 올바르지 않은 숫자를 입력해서 명령에 실패했습니다.`)
        return interaction.reply({ ephemeral: true, embeds: [embed3]})}
      if (money1 < betting) {
        Sublogs.send(`**법인에서개인이체 실패** <@${interaction.user.id}>님이 가진돈보다 많이 입력해서 명령에 실패했습니다.`)
        return interaction.reply({ ephemeral: true, embeds: [embed4]})}
      if (!findMe.account == 0) {
        Sublogs.send(`**법인에서개인이체 실패** <@${interaction.user.id}>님이 보내는분 계좌가 정지되어 있어서 명령에 실패했습니다.`)
        return interaction.reply({ ephemeral: true, embeds: [embed6]})}
      if (!findSend.account == 0) {
        Sublogs.send(`**법인에서개인이체 실패** <@${interaction.user.id}>님이 받는분 계좌가 정지되어 있어서 명령에 실패했습니다.`)
